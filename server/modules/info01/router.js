module.exports = function (router,body,connection){
    ...
    router.post(`/api/userinfo_01/`,body,ctx => {
        ...
        if(info.person != null && info.home_address != null && info.work_address!=null){
            connection.query(`update user_base_info set personName="${info.person}",work_address ="${info.work_address}",home_address="${info.home_address}" where phoneNumber = "${info.phoneNumber}"`)
        }
